...
): { amount: number; alternativeDay: boolean } => {
  const rate = isPublicHoliday ? HOLIDAY_PAY_RATES.TIME_AND_A_HALF : HOLIDAY_PAY_RATES.REGULAR;
  return {
    amount: hourlyRate * hoursWorked * rate,
    alternativeDay: isPublicHoliday && includeAlternativeDay && HOLIDAY_PAY_RATES.ALTERNATIVE_DAY,
  };
};
